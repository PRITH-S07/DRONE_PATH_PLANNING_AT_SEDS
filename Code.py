...
def CartLatfile():
    B=[]
    file = open("c:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/tan_input.txt", "r")
    m=int(file.readline())
    for i in range(0,m):
        p=file.readline()
        imp=tuple(int(float(x.strip())) for x in p.split(','))
        B.append(imp)
    n=int(file.readline())   
    for i in range(0,n):
        p=file.readline()
        inp=tuple(int(float(x.strip())) for x in p.split(','))
        B.append(inp)
    C=CartLat(B)
    a_file = open("c:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/3D.txt", "w")
    for row in C: 
        a_file.write(str(row)[1:-1]+'\n')
    a_file.close()

def LatCartfile():
    import numpy as np
    B=[]
    file =np.loadtxt(open('c:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/3D.txt',"rb"),delimiter=",", skiprows=0)
    df=LatCart(file)
    n_waypoints=5
    n_obstacles=9
    a_file = open("c:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/inputfile.txt", "w")
    a_file.write(str(n_waypoints)+'\n')
    for row in range(0,n_waypoints):
        a_file.write(str(df[row])[1:-1]+'\n')
    a_file.write(str(n_obstacles)+'\n')
    for row in range(n_waypoints,n_obstacles+n_waypoints):
        rad=int(input("Enter radius value:"))
        l1=list(df[row])
        l1.append(rad)
        t1=tuple(l1)
        a_file.write(str(t1)[1:-1]+'\n')
    a_file.close()
...
def draw_tan(st,ed,pos):
    ...
    if(dist>0):
        ...
        sdx1=ar[pos][0]+1.2*(solx1-ar[pos][0])
        sdx2=ar[pos][0]+1.2*(solx2-ar[pos][0])
        sdy1=ar[pos][1]+1.2*(soly1-ar[pos][1])
        sdy2=ar[pos][1]+1.2*(soly2-ar[pos][1])
...
def main():
    ...
    CartLatfile()
    LatCartfile()
    file =open('c:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/inputfile.txt',"r")
    m=int(file.readline())
    for i in range(0,m):
        p=file.readline()
        imp=tuple(int(float(x.strip())) for x in p.split(','))
    ...
    while running:
        while ed_count<m:
            ...
            while(1):
                ...
                pos,count=closest_circ(st,ed)
                if count==0:
                    pygame.draw.line(screen,GREEN,st,ed,1)
                    A.append(st)
                    pygame.display.update()
                    break
                sx1,sx2,sy1,sy2=draw_tan(st,ed,pos)
    ...
    df = pd.read_table('C:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/opp.txt', error_bad_lines=False, delim_whitespace=True)
    df.to_excel('C:/Users/reach/OneDrive/Desktop/output.xlsx', 'Sheet2')
    #print("The original excel sheet ka dataframe \n")
    #print(df)
    #print("Ye rahi original excel")
    #senpai=pd.read_excel('C:/SEDS/output.xlsx')
    #print(senpai)
    myworkbook = openpyxl.load_workbook("C:/Users/reach/OneDrive/Desktop/output.xlsx")
    ...
    myworkbook.save("C:/Users/reach/OneDrive/Desktop/output.xlsx")
    #Now change the excel file to a text file .
 
    print(pd.__version__) 
    excel = pd.read_excel('C:/Users/reach/OneDrive/Desktop/output.xlsx', dtype=object, index_col=0)
    ...
    outfile = open('C:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/output.txt', 'w')
    ...
    p = Path('C:/Users/reach/OneDrive/Desktop/SEDS/SUAS-Ardra--master/output.txt')
